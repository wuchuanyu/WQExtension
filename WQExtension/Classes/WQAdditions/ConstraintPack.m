...
//    }
    ...
    {
        //***constraint's container, private for now***
        NSString* selName = @"container";
        if ([self respondsToSelector:NSSelectorFromString(selName)])
        {
            id(*containerSel)(id, SEL);
            containerSel = (id(*)(id, SEL))[self methodForSelector:NSSelectorFromString(selName)];
            View* view = containerSel(self, NSSelectorFromString(selName));
            if (view && [view isKindOfClass:[View class]])

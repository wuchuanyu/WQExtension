...
@implementation NSJSONSerialization (WQ)
...
{
    return [self JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
}

+ (id)JSONObjectWithData:(NSData *)data options:(NSJSONReadingOptions)opt
{
    return [self JSONObjectWithData:data options:opt error:nil];
}
...
-(NSString*)jsonString
{
    return [NSJSONSerialization stringWithObject:self];
}
